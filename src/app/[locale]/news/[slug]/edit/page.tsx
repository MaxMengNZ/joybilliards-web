...
export default async function EditPost({params}: {params: Promise<{locale: string; slug: string}>}) {
  ...
  async function updatePost(formData: FormData) {
    ...
    await s2.from("posts").update({title, excerpt, content_md}).eq("id", data.id);
  ...
  async function deletePost() {
    "use server";
    const s2 = createSupabaseServerClient();
    await s2.from("posts").delete().eq("id", data.id);
